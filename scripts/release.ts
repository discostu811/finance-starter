...
function appendChangesMd(version: string, changelog: string) {
...
async function main() {
  ...
  // 1) Update headers (idempotent)
  sh(`REL_VERSION=${version} REL_CHANGELOG="${changelog.replace(/"/g, '\\"')}" npx ts-node scripts/update-headers.ts`);

  // 2) Fingerprint repo (code only)
  sh(`npx ts-node scripts/fingerprint.ts`);
  ...
  // 5) Optional: re-fingerprint including artifacts for audit
  sh(`npx ts-node scripts/fingerprint.ts --include-artifacts`);
